/**
 * WPM + accuracy tests — Phase 3 Plan 03 tracer.
 *
 * 10 tests covering D-05 net WPM + D-06 accuracy spec fixtures + edge cases.
 * Spec fixture (30 correct chars / 30s / 0 errors) per D-05 = 12 WPM.
 * ROADMAP criterion 2 says 2 WPM — discrepancy documented in scoring.ts JSDoc.
 */
import { describe, test, expect } from "bun:test";
import { computeNetWpm, computeAccuracy } from "../race/scoring.ts";

describe("computeNetWpm — D-05", () => {
  test("1. SPEC FIXTURE: 30 correct chars / 30s / 0 errors = 12 WPM (D-05 formula). ROADMAP says 2 WPM — discrepancy documented in scoring.ts JSDoc.", () => {
    // (30 / 5) / (30/60) = 6 / 0.5 = 12
  ...
  test("3. clamped at zero: 5 correct / 10 errors / 5s → 0 WPM (not negative)", () => {
    // (5/5 − 10/5) / (5/60) = (1 − 2) / 0.0833 = −12 → clamped to 0
